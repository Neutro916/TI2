...
export const LocalShell: React.FC<LocalShellProps> = ({ type, shellId, initialCommand }) => {
  ...
  useEffect(() => {
    ...
    let history: string[] = [];
    try {
      const savedHistory = localStorage.getItem(`terminal_history_${type}`);
      if (savedHistory) {
    ...
    socket.on('connect', () => {
      ...
      if (initialCommand && !initialCommandExecuted.current) {
        term.writeln(`\x1b[34m[INIT] Executing: ${initialCommand}\x1b[0m`);
        socket.emit('terminal:input', initialCommand + '\n');
    ...
    term.onData((data) => {
      ...
      if (data === '\r') {
        ...
        if (currentLine.trim()) {
          ...
          try {
            localStorage.setItem(`terminal_history_${type}`, JSON.stringify(history));
          } catch (e) {}
        }
